// main.cpp (Completo con los eventos de teclado añadidos)
...
#include <SDL3/SDL_events.h> // Necesario para eventos
...
int main(int, char* [])
{
    ...
    do
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            // --- NUEVO: EVENTOS DE TECLADO ---
            case SDL_EVENT_KEY_DOWN:
            {
                // Enviamos la tecla pulsada a la escena (true)
                scene.on_key(event.key.key, true);

                // Salir con ESC
                if (event.key.key == SDLK_ESCAPE) exit = true;
                break;
            }

            case SDL_EVENT_KEY_UP:
            {
                // Enviamos la tecla soltada a la escena (false)
                scene.on_key(event.key.key, false);
                break;
            }
            // ---------------------------------
